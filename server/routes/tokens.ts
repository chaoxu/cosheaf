...
import { createToken, listTokens, revokeToken } from "../users.js";
import { requireAuth } from "../middleware.js";

export const tokens = new Hono<AppEnv>();
tokens.use("*", requireAuth);

tokens.get("/", (c) => c.json({ tokens: listTokens(c.get("db"), c.get("user").id) }));

tokens.post("/", async (c) => {
  const body = (await c.req.json().catch(() => null)) as { name?: string } | null;
  const name = body?.name?.trim();
  if (!name) return c.json({ error: "name required", code: "validation" }, 400);
  const created = createToken(c.get("db"), c.get("user").id, name);
...
tokens.delete("/:id", (c) => {
  ...
  if (!revokeToken(c.get("db"), c.get("user").id, id))
